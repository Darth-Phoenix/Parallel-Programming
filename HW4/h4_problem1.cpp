...
void *child(void *data){
    ...
    for(int count = 0; count < NSmooth; count++){     
		...
		//進行平滑運算
		for(int i = my_id; i < bmpInfo.biHeight ; i += thread_num){
...
/*********************************************************/
    ...
    if (argc != 2){
        ...
        cout << "Try enter in the format ./[a.out] [thread_num]" << endl;
        exit(0); 
    }

    pthread_t *t;
    //worker thread數量  
    thread_num = strtol(argv[1], NULL, 0);
    ...
    //data紀錄每個thread各自代表的編號(0 ~ thread_num-1)
    ...
    cout << "The execution time = " << diff << " sec." << endl;;   
